module.exports = {
  ...
  code(cache) {
    ...
    this.GetOptionValue("filter", cache) == 1 ? this.GetInputValue("user", cache).bot ? null : (this.RunNextBlock("action", cache)) : this.GetInputValue("user", cache).bot ? (this.RunNextBlock("action", cache)) : null
  }
}